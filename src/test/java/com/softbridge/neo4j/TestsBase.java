...
abstract public class TestsBase {
    ...
    protected Path getDatabasePath() {
        ...
        if (databaseDir==null) {
            System.out.println("Please define the databaseDir property");
        ...
        if (!Files.isWritable(databasePath)) {
            System.out.println("Please define the databaseDir property to a writable directory");
    ...
    protected StringBuffer getDatabaseInitScript() throws IOException {
        Reader in = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("/movie.cypher")));
